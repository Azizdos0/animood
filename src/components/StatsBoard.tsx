...
export function StatsBoard({
  ...
  const listEntries: ListEntry[] = Object.values(entries);

  // Metadata-dependent subset — only ids the media API actually returned.
  const statEntries: StatEntry[] = ids
    .filter((id) => media[id])
    .map((id) => ({ media: media[id], entry: entries[id] }));

  const list = listTotals(listEntries);
  const totals = computeTotals(statEntries);
  const dist = scoreDistribution(listEntries);
  const genres = genreBreakdown(statEntries).slice(0, 10);
  const tags = topTags(statEntries, 10);
  const formats = formatBreakdown(statEntries);
  const statuses = statusBreakdown(listEntries)
    .filter((s) => s.count > 0)
    .map((s) => ({ name: STATUS_LABELS[s.status], count: s.count }));
  const affinity = tasteAffinitySummary(statEntries);
  const titlesSub =

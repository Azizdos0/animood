...
import { createThread, listThreads } from "@/lib/discussions/queries";
...
const ERROR_COPY: Record<"empty" | "too_long" | "unknown", string> = {
  empty: "Title and body are required.",
  too_long: "Title max 200, body max 5000 characters.",
...
function ThreadRow({ mediaId, thread }: { mediaId: number; thread: DiscussionThread }) {
  return (
    <Link
      href={`/media/${mediaId}/discussions/${thread.id}`}
    ...
    >
      <div className="flex items-center justify-between gap-3">
        <p className="min-w-0 truncate text-sm font-semibold">{thread.title}</p>
...
export function DiscussionBoard({ mediaId }: { mediaId: number }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!viewerId || pending || title.trim() === "" || body.trim() === "") return;
    setPending(true);
    setComposerError(null);
    try {
      const result = await createThread(supabaseBrowser(), viewerId, mediaId, title, body);
      if (result.ok) {
        router.push(`/media/${mediaId}/discussions/${result.id}`);
      } else {
  ...
  if (status === "unconfigured") {
    return (
      <div>
        <div className="mono mb-4 text-[11px] tracking-[0.16em] text-violet">DISCUSSIONS</div>
        <p className="text-sm text-muted-foreground">Discussions unavailable.</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      {viewerId ? (
        ...
        <p className="mb-6 text-sm text-muted-foreground">Sign in to start a discussion.</p>
      ) : null}
      ...
      {status === "loading" ? (
        ...
        <p className="rounded-2xl border border-dashed border-border py-12 text-center text-sm text-muted-foreground">
          Couldn&apos;t load discussions right now. Please try again later.
        </p>
      ) : threads.length === 0 ? (
        <p className="rounded-2xl border border-dashed border-border py-12 text-center text-sm text-muted-foreground">
        ...
        <div className="space-y-3">
          {threads.map((thread) => (
            <ThreadRow key={thread.id} mediaId={mediaId} thread={thread} />
          ))}
        </div>
      )}
    </div>
